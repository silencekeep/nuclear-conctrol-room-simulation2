...
namespace EarcutNet
{
    public class Earcut
    {
        public static Mesh triangulate(Mesh inputMesh, List<Vector2> points, Funcs.MeasurementPlane plane, float offset, bool flip)
        {
            if (inputMesh == null)
                inputMesh = new Mesh();
            ...
            {
                if (plane == Funcs.MeasurementPlane.XZ || plane == Funcs.MeasurementPlane.Free)
                    vertices[i] = new Vector3(points[i].x, offset, points[i].y);
                else if (plane == Funcs.MeasurementPlane.XY)
                    vertices[i] = new Vector3(points[i].x, points[i].y, offset);
                else if (plane == Funcs.MeasurementPlane.ZY)
                    vertices[i] = new Vector3(offset, points[i].x, points[i].y);
            }
            ...
            var triangles = Tessellate(data, new int[0]).ToArray();
            var finalTriResult = new List<int>(triangles);
            if (!flip)
            {
                for (int i = 0; i < finalTriResult.Count; i += 3)
                {
                    finalTriResult[i] = triangles[i + 2];
                    finalTriResult[i + 2] = triangles[i];
                }
            }

            inputMesh.vertices = vertices;
            inputMesh.triangles = finalTriResult.ToArray();
