...
VertexOutputForwardClipAdd vertForwardClipAdd (VertexInput v)
{
	...
	#define IN_WORLDPOS_FWDADD_CLIP(i) float3(0,0,0)
#endif
...
fixed f = 1;
	...
	c.rgb = f>=0 ? c.rgb: _SectionColor.rgb;
...
)
{
	...
	bool sampleReflectionsInDeferred = (facing<0);
#else
	...
	if(facing<0) emissiveColor = float4(1,1,1,1) -_SectionColor;

	UnityStandardData data;
	data.diffuseColor = (facing>0)?s.diffColor:_SectionColor ;
	data.occlusion = (facing>0)?occlusion:float4(0,0,0,1);
	data.specularColor = (facing>0)?s.specColor:float4(0,0,0,1);
	data.smoothness = (facing>0)?s.smoothness:float4(0,0,0,1);
	data.normalWorld = (facing>0)?s.normalWorld:float4(0,0,0,1);
