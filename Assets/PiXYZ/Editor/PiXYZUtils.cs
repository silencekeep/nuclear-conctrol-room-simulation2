...
namespace PiXYZ
{
    class PiXYZUtils
    {
        ...
        [MenuItem("PiXYZ/Check Compatibility")]
        public static void showCompatibilityIssues()
        {
            string details = "";
            if (!checkCompatibility(out details))
                EditorUtility.DisplayDialog("PiXYZ Compatibility", "PiXYZ Plugin is not compatible with your setup !\n\nDetails: " + details, "ok");
            else
                EditorUtility.DisplayDialog("PiXYZ Compatibility", "PiXYZ Plugin is fully compatible with your setup !", "ok");
        }

        public static bool checkCompatibility(out string details)
        {
            details = "";
#if !UNITY_2018_1_OR_NEWER
        details = "PiXYZ Plugin is not compatible with your Unity version, please update Unity.\n";
#else
            if (!SystemInfo.operatingSystem.Contains("Windows") && !SystemInfo.operatingSystem.Contains("Linux"))
                details += "PiXYZ plugin is only compatible on Windows OS\n";
#endif
            return details == "";
        }
        ...
        void printSettingsGUI(SerializedObject serializedObject, string prefix = PiXYZSettings.serializePrefix, string fileExt = "", GameObject gameObject = null)
        {
            ...
            int lastShown = 0;
            scrollViewPosition = GUILayout.BeginScrollView(scrollViewPosition, GUILayout.MinHeight(120), GUILayout.MaxHeight(Screen.height - 30));
            {
                float width = GUI.skin.label.CalcSize(new GUIContent("Merge final assemblies")).x;
                g_scrollViewPosition[winId] = scrollViewPosition;
                ...
                EditorGUILayout.BeginVertical();
                {
                    ...
                    beginGroupBox("Coordinate System", isInspector, tt);
                    {
                        ...
                        EditorGUILayout.BeginHorizontal();
                        {
                            ...
                            GUILayout.Label("Scale", GUILayout.Width(width));
                            rect = EditorGUILayout.GetControlRect();
                            serializedProperty.FindPropertyRelative("scaleFactor").floatValue = EditorGUI.FloatField(rect, serializedProperty.FindPropertyRelative("scaleFactor").floatValue);
                            GUILayout.Space(10);
                        }
                        ...
                        EditorGUILayout.BeginHorizontal();
                        {
                            ...
                            GUILayout.Label("Right Handed", GUILayout.Width(width));
                            rect = EditorGUILayout.GetControlRect();
                            serializedProperty.FindPropertyRelative("isRightHanded").boolValue = EditorGUI.Toggle(rect, serializedProperty.FindPropertyRelative("isRightHanded").boolValue);
                        }
                        EditorGUILayout.EndHorizontal();
                        EditorGUILayout.BeginHorizontal();
                        {
                            ...
                            GUILayout.Label("Z-up", GUILayout.Width(width));
                            rect = EditorGUILayout.GetControlRect();
                            serializedProperty.FindPropertyRelative("isZUp").boolValue = EditorGUI.Toggle(rect, serializedProperty.FindPropertyRelative("isZUp").boolValue);
                        }
                        EditorGUILayout.EndHorizontal();
                        EditorGUILayout.Space();
                    }
                    ...
                    if (!isPiXYZExt(fileExt))
                    {
                        ...
                        beginGroupBox("Import Settings", isInspector, tooltip: tt);
                        {
                            ...
                            EditorGUILayout.BeginHorizontal();
                            {
                                ...
                                GUILayout.Label("Hierarchy mode", GUILayout.Width(width));
                                ...
                                propertyNames.Add("Merge final assemblies");
                            ...
                            EditorGUILayout.EndHorizontal();
                            if ((TreeProcessType)serializedProperty.FindPropertyRelative("treeProcess").intValue == TreeProcessType.NONE)
                            {
                                EditorGUILayout.BeginHorizontal();
                                {
                                    GUILayout.Space(20);
                                    GUILayout.Label(new GUIContent("Load Metadata", "If checked, metadata is imported as properties within Unity, and visible in the Inspector window for each object in the hierarchy. Metadata can be imported only when the full product hierarchy is imported (meaning Hierarchy Optimization mode = None).\nA metadata is a piece of information that can be carried by any node of a CAD model’s hierarchy. "), GUILayout.Width(width));
                                    //List<string> propertyNames = new List<string>(3);
                                    //propertyNames.Add("Don't load");
                                    //propertyNames.Add("Load from parent Instance if there is no Metadata on node");
                                    //propertyNames.Add("Ignore instances' metadata");
                                    //propertyNames.Add("Load all Metadata");
                                    //List<int> intValue = new List<int>(3);
                                    //intValue.Add(0);
                                    //intValue.Add(1);
                                    //intValue.Add(2);
                                    //intValue.Add(3);
                                    //Rect rect = EditorGUILayout.GetControlRect();
                                    //serializedProperty.FindPropertyRelative("loadMetadata").intValue = EditorGUI.IntPopup(rect, serializedProperty.FindPropertyRelative("loadMetadata").intValue, propertyNames.ToArray(), intValue.ToArray());
                                    serializedProperty.FindPropertyRelative("loadMetadata").intValue = EditorGUILayout.Toggle(serializedProperty.FindPropertyRelative("loadMetadata").intValue != 0, GUILayout.Width(40)) ? 1 : 0;
                                    GUILayout.FlexibleSpace();
                                }
                                EditorGUILayout.EndHorizontal();
                            }else
                            {
                                serializedProperty.FindPropertyRelative("loadMetadata").intValue = 0;
                            }
                            //EditorGUILayout.BeginHorizontal();
                            //{
                            //    GUILayout.Space(20);
                            //    tt = PiXYZUtils.getTooltipText<PiXYZSettings>("useMergeFinalAssemblies");
                            //    GUILayout.Label(new GUIContent("Merge final assemblies", tt));
                        ...
                        if (!serializedObject.FindProperty("settings.useLods").boolValue)
                        {
                            ...
                            beginGroupBox("Mesh Quality", isInspector, tooltip: tt);
                            {
                                ...
                                {
                                    GUILayout.Space(20);
                                    tt = "Choose your quality level";
                                    GUILayout.Label(new GUIContent("Quality", tt));
                                    EditorGUILayout.PropertyField(serializedObject.FindProperty("settings.lodSettings.list").GetArrayElementAtIndex(0).FindPropertyRelative("Quality"), GUIContent.none);
                                    EditorGUILayout.BeginHorizontal();
                                    {
                                        GUILayout.Space(5);
                        ...
                        beginGroupBox("Post Process", isInspector, tooltip: "Generate UV : Use this setting to add a new primary UV set (channel #0). Set the size of the projection box used to create UVs.\n\nCaution: PiXYZ will override the existing UV set, do not Use this setting if you wish to preserve the UVs embedded in the imported model.\n\nOrient faces consistently… : Use this setting for PiXYZ to perform a unification of all triangles orientation.\n\nCaution: Do not Use this setting if the imported model is a mesh (tessellated geometry) and is already correctly oriented.");
                        {
                            EditorGUILayout.BeginHorizontal();
                            {
                                ...
                                //GUILayout.FlexibleSpace();
                                EditorGUI.BeginDisabledGroup(!serializedProperty.FindPropertyRelative("mapUV").boolValue);
                                {
                            ...
                            GUILayout.BeginHorizontal();
                            {
                                ...
                                GUILayout.Label(new GUIContent("Orient faces consistently", tt), GUILayout.ExpandWidth(true));
                                GUILayout.FlexibleSpace();
                            }
                            ...
                            EditorGUILayout.BeginHorizontal();
                            {
                                tt = PiXYZUtils.getTooltipText<PiXYZSettings>("createPrefab");
                                serializedProperty.FindPropertyRelative("createPrefab").boolValue = EditorGUILayout.Toggle(serializedProperty.FindPropertyRelative("createPrefab").boolValue, GUILayout.Width(40));
                                GUILayout.Label(new GUIContent("Create prefab", tt));
                                GUILayout.FlexibleSpace();
                            }
                            EditorGUILayout.EndHorizontal();
                        }
