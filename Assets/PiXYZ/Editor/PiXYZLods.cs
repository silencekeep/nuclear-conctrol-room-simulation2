...
namespace PiXYZ
{
    ...
    class PiXYZLods
    {
            ...
            PiXYZUtils.beginGroupBox("LODs Mesh Quality", tooltip: tt);
            {
                ...
                if (!isInspector)
                {
                    GUILayout.BeginHorizontal();
                    {
                        ...
                        {
                            if (serializedProperty.FindPropertyRelative("useLods").boolValue)
                                printLoDSettings(currentLodIndex, serializedObject, prefix + "." + "lodSettings.list");
                            serializedObject.ApplyModifiedProperties();
                            GUILayout.Space(10);
                        }
                                ...
                                foreach (var lodGroup in gameObject.GetComponentsInChildren<LODGroup>())
                                {
                                    for (int i = 1; i < lodGroup.lodCount; ++i)
                                    {
                                        if (lodGroup.GetLODs()[0].renderers.Length != lodGroup.GetLODs()[i].renderers.Length)
                                            Debug.LogError("The number of renderers on each LOD is not equal, can't synchronize !");
                                        else
        ...
        public static void printLoDSettings(int index, SerializedObject serializedObject, string prefix = "settings.lodSettings.list", bool showTessellation = true)
        {
            int winId = serializedObject.FindProperty("windowId").intValue;
            string tt = "";
            serializedObject.Update();
            SerializedProperty lodProperties = serializedObject.FindProperty(prefix+".Array");
            EditorGUILayout.BeginHorizontal();
            {
                EditorGUILayout.BeginVertical();
                {
                    ...
                    {
                        if (lodProperties.GetArrayElementAtIndex(i).FindPropertyRelative("Quality").intValue > 4)
                        {
                        ...
                        EditorGUILayout.BeginHorizontal();
                        {
                            ...
                            EditorGUILayout.PropertyField(lodProperties.GetArrayElementAtIndex(i).FindPropertyRelative("Quality"), GUIContent.none, GUILayout.ExpandWidth(true));
                            for (int j = i; j < lodProperties.arraySize - 1; j++)
                                while (lodProperties.GetArrayElementAtIndex(j).FindPropertyRelative("Quality").intValue >= lodProperties.GetArrayElementAtIndex(j + 1).FindPropertyRelative("Quality").intValue)
                                {
                                    lodProperties.GetArrayElementAtIndex(j + 1).FindPropertyRelative("Quality").intValue++;
                                }
                    ...
                    if (lodProperties.GetArrayElementAtIndex(lodProperties.arraySize - 1).FindPropertyRelative("Quality").intValue < 4)
                    {
                        EditorGUILayout.BeginHorizontal();
                        {
                            GUILayout.FlexibleSpace();
        ...
        {
            SerializedProperty serializedProperty = serializedObject.FindProperty(prefix);
            PiXYZLoDSettingsEditor.removeAt(index, serializedObject);
            serializedProperty.FindPropertyRelative("lodSettingCount").intValue = serializedProperty.FindPropertyRelative("lodSettings.list").arraySize;
        ...
        static public void addLod(int index, SerializedObject serializedObject, int insertModel, double insertStartLod, string prefix)
        {
            SerializedProperty serializedProperty = serializedObject.FindProperty(prefix);
            ...
            if (insertStartLod >= 0f)
                serializedProperty.FindPropertyRelative("lodSettings.list").GetArrayElementAtIndex(insertModel).FindPropertyRelative("Threshold.v").doubleValue = insertStartLod;
            serializedObject.ApplyModifiedProperties();
            PiXYZSettingsEditor.saveEditorPref(serializedObject, prefix);
        }
