...
namespace PiXYZ
{
    class PiXYZLODSlider
    {
        ...
        public void show(SerializedObject serializedObject, GameObject gameObject=null)
        {
            ...
            float culledValue = (float)PiXYZLoDSettingsEditor.getIndexProperty(maxLod-1, serializedObject, "Threshold.v", "settings.lodSettings.list").doubleValue;
            float lodValue = 1f;
            float nextLodValue = (float)PiXYZLoDSettingsEditor.getIndexProperty(0, serializedObject, "Threshold.v", "settings.lodSettings.list").doubleValue;
            float start = 0;
            for (int i = 0; i < maxLod; i++)
            {
                labelRect.width = (sliderRect.width - culledWidth * culledValue) * (lodValue - nextLodValue);
                ...
                else if (grabbing!= -1 && Event.current.type == EventType.MouseUp && Event.current.button == 0)
                {
                    if (gameObject != null)
                    {
                        foreach (LODGroup lodGroup in gameObject.GetComponentsInChildren<LODGroup>())
                        {
                            ...
                            lods[grabbing].screenRelativeTransitionHeight = (float)PiXYZLoDSettingsEditor.getIndexProperty(grabbing, serializedObject, "Threshold.v", "settings.lodSettings.list").doubleValue;
                ...
                nextLodValue = i < maxLod - 1 ? (float)PiXYZLoDSettingsEditor.getIndexProperty(i + 1, serializedObject, "Threshold.v", "settings.lodSettings.list").doubleValue : -1;
                start += labelRect.width;
            }
            labelRect.width = (sliderRect.width + culledWidth * culledValue) - start ;
            labelRect.position = new Vector2(sliderRect.position.x + start, sliderRect.position.y);
            PiXYZUtils.GUIDrawRect(labelRect,
            ...
            if (grabbing != -1)
            {
                if (grabbing == maxLod-1 && currentValue < 0.01f)
                    return;
                else if(grabbing==0 && currentValue>0.99f)
                    return;
                else if (grabbing > 0 && currentValue > PiXYZLoDSettingsEditor.getIndexProperty(grabbing - 1, serializedObject, "Threshold.v", "settings.lodSettings.list").doubleValue - 0.01f)
                    return;
                else if (grabbing < maxLod - 1 && currentValue < PiXYZLoDSettingsEditor.getIndexProperty(grabbing + 1, serializedObject, "Threshold.v", "settings.lodSettings.list").doubleValue + 0.01f)
                    return;
                PiXYZLoDSettingsEditor.getIndexProperty(grabbing, serializedObject, "Threshold.v", "settings.lodSettings.list").doubleValue = currentValue<=0f?1f: currentValue;
            }
        }
    }
}