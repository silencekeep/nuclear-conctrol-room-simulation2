...
namespace PiXYZ
{
    ...
    public class PiXYZSettingsInspector : Editor
    {
        ...
        public IEnumerator ImportModel()
        {
            ...
            loader.configure(settings.settings.useScaleOnTolerances ? settings.settings.scaleFactor : 1, settings.settings.orient, mapUV3dSize, settings.settings.treeProcess, settings.settings.useLods ? settings.settings.lodsMode : LODsMode.NONE, settings.settings.lodSettings, !settings.settings.splitTo16BytesIndex, settings.settings.useMergeFinalAssemblies, settings.settings.loadMetadata, settings.settings.createPrefab);

            CoroutineNode coco = coroutineScheduler.StartCoroutine(loader.loadFileRuntime(gameObject, settings.settings.originalFilename, true));
            yield return coco;
			
            loader.setProgress("Creating prefab...", 0.9f);
