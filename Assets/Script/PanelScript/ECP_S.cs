...
namespace CNCC.Panels
{
    public class ECP_S : Panel, IOWP
    {
        ...
        public static List<ECP_S> ALLECP_Ss = new List<ECP_S>();
        //public string ID { get; set; }
        GameObject generatedECP_S;
        [SerializeField] float objectHeight = 1.140f;
        public ECP_S(GameObject ECP_SPrefab, Transform spawnPoint, Transform position, string id)
        {
            generatedECP_S = Instantiate(ECP_SPrefab, spawnPoint);
            generatedECP_S.GetComponent<ECP_S>().ID = SetID(id);
            generatedECP_S.gameObject.transform.position = new Vector3(position.position.x, position.position.y + objectHeight / 2, position.position.z);
            ALLECP_Ss.Add(generatedECP_S.GetComponent<ECP_S>());
            Panel.AllPanels.Add(generatedECP_S.GetComponent<ECP_S>());
        }
