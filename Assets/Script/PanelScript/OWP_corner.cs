...
{
    public class OWP_corner : Panel, IOWP
    {
        ...
        public static List<OWP_corner> AllOWP_corner = new List<OWP_corner>();
        //public string ID { get; set; }        
        GameObject generatedOWP_corner;
        [SerializeField] float objectHeight = 1.14f;
        public OWP_corner(GameObject OWP_cornerPrefab, Transform transform, Transform position, string id)
        {
            generatedOWP_corner = Instantiate(OWP_cornerPrefab, transform);
            generatedOWP_corner.GetComponent<OWP_corner>().ID = SetID(id);
            generatedOWP_corner.gameObject.transform.position = new Vector3(position.position.x, position.position.y + objectHeight / 2, position.position.z);
            AllOWP_corner.Add(generatedOWP_corner.GetComponent<OWP_corner>());
            Panel.AllPanels.Add(generatedOWP_corner.GetComponent<OWP_corner>());
        }
        string SetID(string id)
        {
            if (id.Trim() == "")
            {
                id = "OWP拐角台" + AllOWP_corner.Count;
            }
            return id;
        }
        void Start()
        ...
        public override void DeletePanel(Panel panel)
        {
            ...
            AllOWP_corner.Remove((OWP_corner)panel);
        }
        ...
        public void SetmonitorMiddlePositio(Vector3 vector3)
        {
            ...
            part1.GetComponent<OWP_cornerPoint>().SetMiddlePointPosition(vector3);
        }
