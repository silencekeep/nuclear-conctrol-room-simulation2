...
namespace CNCC.Panels
{
    public class ECP_N : Panel, IOWP
    {
        ...
        public static List<ECP_N> ALLECP_Ns = new List<ECP_N>();
        //public string ID { get; set; }
        GameObject generatedECP_N;
        [SerializeField] float objectHeight = 1.140f;
        public ECP_N(GameObject ECP_NPrefab, Transform spawnPoint, Transform position, string id)
        {
            generatedECP_N = Instantiate(ECP_NPrefab, spawnPoint);
            generatedECP_N.GetComponent<ECP_N>().ID = SetID(id);
            generatedECP_N.gameObject.transform.position = new Vector3(position.position.x, position.position.y + objectHeight / 2, position.position.z);
            ALLECP_Ns.Add(generatedECP_N.GetComponent<ECP_N>());
            Panel.AllPanels.Add(generatedECP_N.GetComponent<ECP_N>());
        }
