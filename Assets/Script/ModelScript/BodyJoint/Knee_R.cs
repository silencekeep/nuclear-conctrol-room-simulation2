...
{
    public class Knee_R: MonoBehaviour, IJoint
    {
        ///个性化内容
        //初始边界范围
    ...
    float z_LimitMin_Min = -1;
        ...
        {
            return model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.RightLowerLeg).gameObject.transform.localEulerAngles;
        }
        ...
        {
            model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.RightLowerLeg).gameObject.transform.localEulerAngles = angle;
...
(x, y, z) = CheckAngleValue(x, y, z);
            model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.RightLowerLeg).localEulerAngles = new Vector3(x, y, z);
