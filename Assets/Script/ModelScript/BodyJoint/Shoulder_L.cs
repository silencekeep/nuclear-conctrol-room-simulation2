...
{
    public class Shoulder_L : MonoBehaviour,IJoint
    {
        //边界最大值
        ...
        float y_LimitMax_Max = 182;
        float y_LimitMax_Min = 110;
        float y_LimitMin_Max = -18;
        float y_LimitMin_Min = -54;
        float z_LimitMax_Max = 153;
        float z_LimitMax_Min = 91;
        float z_LimitMin_Max = -25;
        float z_LimitMin_Min = -60;
        //初始边界范围
        static float x_max = 61;
        static float x_min = -180;
        static float y_max = 140;
        static float y_min = -34;
        static float z_max = 134;
        static float z_min = -48;
        ...
        {
            return model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.LeftUpperArm).gameObject.transform.localEulerAngles;
        }
        ...
        {
            model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.LeftUpperArm).gameObject.transform.localEulerAngles = angle;
...
(x, y, z) = CheckAngleValue(x, y, z);
            model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.LeftUpperArm).localEulerAngles = new Vector3(x, y, z);
