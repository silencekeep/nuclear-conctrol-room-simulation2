...
{
    public class Wrist_L : MonoBehaviour, IJoint
    {
        ///个性化内容
        //边界最大值
        ...
        float y_LimitMax_Max = 50;
        float y_LimitMax_Min = 24;
        float y_LimitMin_Max = -31;
        float y_LimitMin_Min = -55;
        float z_LimitMax_Max = 115;
        float z_LimitMax_Min = 70;
        float z_LimitMin_Max = -82;
        float z_LimitMin_Min = -130;
        //初始边界范围
        static float x_max = 0;
        static float x_min = -1;
        static float y_max = 30;
        static float y_min = -44;
        static float z_max = 87;
        static float z_min = -104;
        ...
        {
            return model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.LeftHand).gameObject.transform.localEulerAngles;
        }
        ...
        {
            model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.LeftHand).gameObject.transform.localEulerAngles = angle;
...
(x, y, z) = CheckAngleValue(x, y, z);
            model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.LeftHand).localEulerAngles = new Vector3(x, y, z);
