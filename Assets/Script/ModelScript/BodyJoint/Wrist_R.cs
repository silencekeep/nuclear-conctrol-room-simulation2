...
{
    public class Wrist_R : MonoBehaviour, IJoint
    {
        ///个性化内容
        //边界最大值
        ...
        float y_LimitMax_Max = 55;
        float y_LimitMax_Min = 31;
        float y_LimitMin_Max = -24;
        float y_LimitMin_Min = -50;
        float z_LimitMax_Max = 130;
        float z_LimitMax_Min = 82;
        float z_LimitMin_Max = -70;
        float z_LimitMin_Min = -115;
        //初始边界范围
        static float x_max = 0;
        static float x_min = -1;
        static float y_max = 44;
        static float y_min = -30;
        static float z_max = 104;
        static float z_min = -87;
        ...
        {
            return model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.RightHand).gameObject.transform.localEulerAngles;
        }
        ...
        {
            model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.RightHand).gameObject.transform.localEulerAngles = angle;
...
(x, y, z) = CheckAngleValue(x, y, z);
            model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.RightHand).localEulerAngles = new Vector3(x, y, z);
