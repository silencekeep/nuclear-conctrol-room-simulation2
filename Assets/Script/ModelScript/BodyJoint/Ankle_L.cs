...
{
    public class Ankle_L : MonoBehaviour, IJoint
    {
        ///个性化内容
        //初始边界范围
        ...
        static float y_min = -42;
        static float z_max = 30;
        ...
        float y_LimitMax_Max = 40;
        float y_LimitMax_Min = 15;
        float y_LimitMin_Max = -23;
        float y_LimitMin_Min = -58;
        float z_LimitMax_Max = 55;
        float z_LimitMax_Min = 17;
        float z_LimitMin_Max = -10;
        float z_LimitMin_Min = -49;
        ...
        {
            return model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.LeftFoot).gameObject.transform.localEulerAngles;
        }
        ...
        {
            model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.LeftFoot).gameObject.transform.localEulerAngles = angle;
        ...
        public void SetCurrentAngle(GameObject model, float x, float y, float z)
        {
            ...
            model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.LeftFoot).gameObject.transform.localEulerAngles = new Vector3(x,y,z);
