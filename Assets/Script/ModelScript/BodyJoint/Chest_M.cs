...
{
    public class Chest_M : MonoBehaviour, IJoint
    {
        ///个性化内容
        //初始边界范围
        static float x_max = 80;
        static float x_min = -25;
        static float y_max = 20;
        static float y_min = -20;
        static float z_max = 50;
        static float z_min = -50;
        //边界最大值
        float x_LimitMax_Max = 98;
        float x_LimitMax_Min = 71;
        float x_LimitMin_Max = -19;
        float x_LimitMin_Min = -38;
        float y_LimitMax_Max = 32;
        float y_LimitMax_Min = 15;
        float y_LimitMin_Max = -15;
        float y_LimitMin_Min = -35;
        float z_LimitMax_Max = 65;
        float z_LimitMax_Min = 38;
        float z_LimitMin_Max = -38;
        float z_LimitMin_Min = -61;
        float x_current;
        float y_current;
        float z_current;
        //该脚本对应的关节
        ...
        {
            return model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.Chest).gameObject.transform.localEulerAngles;
        }
        ...
        {
            model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.Chest).gameObject.transform.localEulerAngles = angle;
...
(x, y, z) = CheckAngleValue(x, y, z);
            model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.Chest).localEulerAngles = new Vector3(x, y, z);
