...
{
    public class Hip_L : MonoBehaviour,IJoint
    {
        ///个性化内容
        ...
        float y_LimitMax_Max = 60;
        float y_LimitMax_Min = 25;
        float y_LimitMin_Max = -20;
        float y_LimitMin_Min = -45;
        float z_LimitMax_Max = 45;
        float z_LimitMax_Min = 13;
        float z_LimitMin_Max = -34;
        float z_LimitMin_Min = -75;
        //初始边界范围
        static float x_max = 20;
        static float x_min = -109;
        static float y_max = 43;
        static float y_min = -30;
        static float z_max = 28;
        static float z_min = -53;
        ...
        {
            return model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.LeftUpperLeg).gameObject.transform.localEulerAngles;
        }
        ...
        {
            model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.LeftUpperLeg).gameObject.transform.localEulerAngles = angle;
...
(x, y, z) = CheckAngleValue(x, y, z);
            model.GetComponent<Animator>().GetBoneTransform(HumanBodyBones.LeftUpperLeg).localEulerAngles = new Vector3(x, y, z);
