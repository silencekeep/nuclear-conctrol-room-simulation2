...
public class AddType1DropDown : MonoBehaviour
{
   ...
   // public InputField AddTypeInputField;
    ...
    public void Type1TextFilled()//添加动素界面选择动素种类时调用
    {
        AddType1InputField.text = AddType1Dropdown.captionText.text;
    }
    ...
    void Update()
    {
        ...
        //string Seletedtype = QTypeDropdown.captionText.text;
        SqlDataAdapter DC = new SqlDataAdapter("select distinct ActGenType from actions", sqlCon);
        ...
        if (AddType1Dropdown.value == 0)
        {
            AddType1Dropdown.captionText.text = dropdown.options[0].text;
            if (firstload == true)
            {
                firstload = false;
                AddType1InputField.text = AddType1Dropdown.captionText.text;
