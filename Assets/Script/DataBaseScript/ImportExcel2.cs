...
public class ImportExcel2 : MonoBehaviour
{
    /// <summary>
    /// 将excel导入到datatable
    /// </summary>
    ...
    public void ExcelToDataTable() //采标数据
    {
        try
        {
            ...
            if (workbook != null)
            {
                for (int k = 0; k < ExcelTable.Rows.Count; k++)   //将数组中的数据取出显示在下拉列表中
                {
                    ...
                    standardname = Convert.ToString(ExcelTable.Rows[k][0]);
                    item = Convert.ToString(ExcelTable.Rows[k][1]);
                    type = Convert.ToString(ExcelTable.Rows[k][2]);
                    type1 = Convert.ToString(ExcelTable.Rows[k][3]);
                    contents = Convert.ToString(ExcelTable.Rows[k][4]);
                    upper = Convert.ToString(ExcelTable.Rows[k][5]);
                    lower = Convert.ToString(ExcelTable.Rows[k][6]);
                    
                    string ActInsert = "insert into standard (standardname,item,type,type1,contents,upper,lower) values ('" + standardname + "','" + item + "','" + type + "','" + type1 + "','" + contents + "','" + upper + "','" + lower +"')";
